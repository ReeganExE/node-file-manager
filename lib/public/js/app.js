...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location', 'download',
  function ($scope, $http, $location, download) {
    var FM = this;
    ...
    FM.downloadSingle = function(file) {
      download(file);
    }
  ...
  return function download(file) {
      var link = document.createElement('a');
      link.style.display = 'none';
      document.body.appendChild(link);

      for (var i in FM.selection) {
        link.setAttribute('href', 'api' + file.relPath);
